...
def track_acc_GIDS( g, args, label_array=None):
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        for step, (input_nodes, seeds, blocks, ret) in enumerate(train_dataloader):
            ...
            #     print(f"Rank: {rank} step: {step} model_time: {model_time - train_start} loss_time: {back_ward_time - model_time} opt_time: {optimizer_time - back_ward_time} gpu_mem_alloc: {gpu_mem_alloc} MB")
            # num_step += 1
        ...
        print(f"num_step: {num_step} train_time: {train_time} ")
        batch_input_time = 0
        transfer_time = 0
        train_time = 0
        e2e_time = 0
...
if __name__ == '__main__':
   ...
   # g.ndata['features'] = g.ndata['feat']
    ...
    # rank = comm.Get_rank()
    # size = comm.Get_size()
    #print(f"MPI4PY rank: {rank} out of {size} processes")
