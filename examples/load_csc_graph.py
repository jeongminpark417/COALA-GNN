...
if __name__ == '__main__':
    ...
    if(args.data == 'IGB'):
        print("Dataset: IGB")
        indptr_path = osp.join(args.path, args.dataset_size, 'processed', 'paper__cites__paper', 'csc_indptr.npy')
        indices_path = osp.join(args.path, args.dataset_size, 'processed', 'paper__cites__paper', 'csc_indices.npy')
        edge_ids_path = osp.join(args.path, args.dataset_size, 'processed', 'paper__cites__paper', 'csc_edge_ids.npy')

        indptr = torch.from_numpy(np.load(indptr_path))
        indices = torch.from_numpy(np.load(indices_path))
        edge_ids = torch.from_numpy(np.load(edge_ids_path))
        ...
        print(f"Indptr shape: {indptr.dtype} indicies shape:{indices.dtype} Edge id shape: {edge_ids.dtype}")

        graph = dgl.graph(('csc',(indptr, indices, edge_ids)), num_nodes=n_nodes)
        ...
        indptr_path = osp.join(args.path, 'raw', 'csc_indptr.npy')
        indices_path = osp.join(args.path, 'raw', 'csc_indices.npy')
        edge_ids_path = osp.join(args.path, 'raw', 'csc_edge_ids.npy')
